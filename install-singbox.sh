...
OPENRC
        ...
        rc-service sing-box restart || { err "服务启动失败"; tail -20 /var/log/sing-box.err 2>/dev/null || tail -20 /var/log/sing-box.log 2>/dev/null || true; exit 1; }
        sleep 2
        rc-service sing-box status >/dev/null 2>&1 && info "✅ OpenRC 服务已启动" || err "服务状态异常"
    else
...
SYSTEMD
        ...
        systemctl restart sing-box || { err "服务启动失败"; journalctl -u sing-box -n 30 --no-pager; exit 1; }
        sleep 2
        systemctl is-active sing-box >/dev/null 2>&1 && info "✅ Systemd 服务已启动" || { err "服务状态异常"; systemctl status sing-box --no-pager; exit 1; }
    fi
...
[ "$PUB_IP" = "YOUR_SERVER_IP" ] && warn "无法获取公网 IP，请手动替换" || info "检测到公网 IP: $PUB_IP"
...
echo ""
info "🔗 客户端链接："
generate_uri | while IFS= read -r line; do echo "   $line"; done
echo ""
info "🔧 管理命令："
if [ "$OS" = "alpine" ]; then
...
action_uninstall() { info "正在卸载 sing-box..."; service_stop || true; [ "$OS" = "alpine" ] && { rc-update del "$SERVICE_NAME" default >/dev/null 2>&1; [ -f "/etc/init.d/$SERVICE_NAME" ] && rm -f "/etc/init.d/$SERVICE_NAME"; } || { systemctl stop "$SERVICE_NAME" >/dev/null 2>&1 || true; systemctl disable "$SERVICE_NAME" >/dev/null 2>&1 || true; [ -f "/etc/systemd/system/$SERVICE_NAME.service" ] && rm -f "/etc/systemd/system/$SERVICE_NAME.service"; systemctl daemon-reload >/dev/null 2>&1 || true; }; rm -rf /etc/sing-box /var/log/sing-box.log /var/log/sing-box.err "$SS_URI_PATH" "$BIN_PATH" >/dev/null 2>&1 || true; info "卸载完成"; }
